...
public class GenericProjectile
{
    ...
    {
        if (projectilePoint.y < firingUnit.getBackground().hillGround(projectilePoint.x + Main.screenWidth / 2))
        {
            return false;
        } else
        {
    ...
    {
        if (target.getBoundingBox().intersects(projectileBounds))
        {
            return true;
        }
        return false;
    ...
    {
        if (target.getBoundingCircle().intersects(projectileBounds))
        {
            return true;
        }
        return false;
	...
	public boolean shouldMakeParticle()
	{
	...
	public GenericUnit getFiringUnit()
	{
		return firingUnit;
	}

	public Point getProjectilePoint()
	{
		return projectilePoint;
	}
}
