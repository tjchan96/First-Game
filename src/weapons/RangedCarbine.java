...
public class RangedCarbine extends GenericWeapon
{
    ...
    public void paintSelf(Graphics2D g2, Point playerPoint, Color color)
    {
        ...
        {
            firingPoint = new Point((int) ((holder.getUnitPoint().x + holder.unitWidth / 2 - 45) + 80 * Math.cos(Math.toRadians(getDirectionToTarget()))), (int) ((holder.getUnitPoint().y + holder.unitHeight / 4 - 15) + 80 * Math.sin(Math.toRadians(getDirectionToTarget()))));
        } else
        {
            firingPoint = new Point((int) ((holder.getUnitPoint().x + holder.unitWidth / 2 + 45) + 80 * Math.cos(Math.toRadians(getDirectionToTarget()))), (int) ((holder.getUnitPoint().y + holder.unitHeight / 4 - 15) + 80 * Math.sin(Math.toRadians(getDirectionToTarget()))));
        }
