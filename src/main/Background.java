...
//        weaponStrings.add("grenade launcher - for some reason its after auto shotgun");
    ...
    public void paintSelf(Graphics2D g2, Point playerPoint)
    {
        if (!Main.isRunShop())
        {
            ...
            if (timeOfDay > 24)
            {
                timeOfDay = 0;
                day++;
                main.getUnitHolder().getPlayer().setHealth(main.getUnitHolder().getPlayer().getMaxHealth());
                if (day == 4)
                {
    ...
    private void paintSelfHillGround(Graphics2D g2, Point playerPoint)
    {
        ...
        g2.drawString(main.getUnitHolder().getPlayer().getWeapon().getClass().getName().substring(11), (int) (Main.screenWidth / 2 - 200), (int) (Main.screenHeight / 2 - 150));
    }
