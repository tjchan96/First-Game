...
public class GenericWeapon
{
    ...
    public void run()
    {
        ...
        if (!holderPointSet)
        {
            holderPoint = holder.unitPoint;
        }
        targetPoint = holder.targetPoint;
        shouldAttack();
    }
    ...
    public void paintSelf(Graphics2D g2, Point playerPoint, Color color)
    {
        ...
        firingPoint = new Point((int) ((holder.unitPoint.x + holder.unitWidth / 2) + 1 * Math.cos(Math.toRadians(getDirectionToTarget()))), (int) ((holder.unitPoint.y + holder.unitHeight / 4) + 1 * Math.sin(Math.toRadians(getDirectionToTarget()))));
