...
public class ProjectileHolder
{
    ...
    public void paintSelfProjectiles(Graphics2D g2)
    {
        for (int i = 0; i < projectileList.size(); i++)
        {
            projectileList.get(i).paintSelf(g2, main.getUnitHolder().getPlayer().unitPoint);
        }
        g2.setColor(Color.yellow);
        g2.setFont(new Font("showcard gothic", Font.PLAIN, 12));
        Iterator iter = headshotList.iterator();
        while (iter.hasNext())
        {
            Point point = (Point) iter.next();
            g2.drawString("Headshot", point.x - 50 - main.getUnitHolder().getPlayer().unitPoint.x + Main.screenWidth / 2, point.y);
            point.y -= 2;
            if (point.y <= 0)
            {
                iter.remove();
        ...
        Iterator iter2 = invincibleList.iterator();
        while (iter2.hasNext())
        {
        ...
        while (projectileIterator.hasNext())
        {
            ...
            if (!(projectile.noCollision))
            {
                if (projectile.hasHitGround())
                {
                    if (projectile.shouldMakeParticle)
                    {
                        ...
                        {
                            makeHugeSmoke(projectile.firingUnit, projectile.getPoint(), true, 30);
                            makeFire(projectile.firingUnit, projectile.getPoint(), true, 30);
                        }
                        if (projectile instanceof NukeProjectile)
                        {
                            makeMassiveSmoke(projectile.firingUnit, projectile.getPoint(), true, 150);
                            makeMassiveFire(projectile.firingUnit, projectile.getPoint(), true, 150);
                        }
            ...
            for (GenericUnit target : main.getUnitHolder().getEnemiesOf(projectile.isIsPlayerProjectile()))
            {
                if (((projectile.hasHitTargetBody(target) || projectile.hasHitTargetHead(target))))
                {
                    ...
                    {
                        target.damageSelfFace(projectile.getDamage(), projectile.projectilePoint);
                        if (!(target instanceof GenericGrave) && !(target instanceof Player))
                        {
                            headshotList.add(new Point(projectile.projectilePoint));
                    ...
                    {
                        target.damageSelf(projectile.getDamage(), projectile.projectilePoint);
                        if (target instanceof StartingGrave)
                        {
                            invincibleList.add(target);
                        }
