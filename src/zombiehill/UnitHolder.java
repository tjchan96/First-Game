...
public class UnitHolder
{
    ...
    public void paintSelfUnits(Graphics2D g2)
    {
        ...
        {
            unitList.get(i).paintSelf(g2, getPlayer().unitPoint);
        }
    }
    ...
    private void handleCollisions()
    {
        for (int i = 0; i < unitList.size(); i++)
        {
            ...
            {
                if (i != j && unitList.get(i).boundingBox.intersects(unitList.get(j).boundingBox))
                {
                    ...
                    {
                        unitList.get(i).moveAway(unitList.get(j).boundingBox);
                        unitList.get(j).moveAway(unitList.get(i).boundingBox);
                    }
                }
            }
        }
